$(function(){
	$('input[data=textcomment]').click(function(){
		...
		//发送ajax
		$.post('/hcomment',{'_token':token,'Eid':Eid,'comment':comment},function(msg){
			if(msg !== 0) {
				//克隆节点		
	...
	$('.response > .media-body > .tijiao > input[value=评论]').click(function(){
		...
		//发送ajax
		$.post('/hcomment',{'_token':token,'Discuss_type':Discuss_type,'Discuss_type2':Discuss_type2,'tijiao_content':tijiao_content,'username':username,'Eid':Eid},function(msg){
			if(msg !== 0) {
